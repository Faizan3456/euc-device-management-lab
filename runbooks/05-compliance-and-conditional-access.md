...
- After that, the Group Policy setting saved successfully, `gpupdate /force` still printed a warning ("Windows failed to apply the MDM Policy settings") but the enrollment task infrastructure was created regardless — a new GUID folder appeared under EnterpriseMgmt with several tasks, including a "Login Schedule created by enrollment client..." task:

  ![Task Scheduler showing the EnterpriseMgmt folder with a newly-created GUID subfolder containing the enrollment tasks](../assets/screenshots/05-task-scheduler-enterprisemgmt-task-created.webp)

  Manually running that task triggered real enrollment — the device appeared in Intune within a minute, already showing **Compliant** by default (no compliance policy exists yet, so it has nothing to fail):

  ![Intune Devices | All devices showing WIN-KH38OBH7, Managed by Intune, Corporate ownership, and Compliant status](../assets/screenshots/05-intune-device-enrolled-compliant.webp)

With the device enrolled, created `Lab-Compliance-Windows` (BitLocker, minimum OS version, password required, Defender Antimalware) and `Lab-Compliance-macOS` (FileVault, jailbreak check, password required — untestable without an enrolled Mac, see [03-macos-ade-enrolment.md](03-macos-ade-enrolment.md)), both assigned to "All devices" since there's currently only one real enrolled device in the tenant. Synced the Windows device and confirmed it evaluated as **Compliant** against the real requirements, not just the "compliant by default, no policy exists" state from before.

Then created `Lab-CA-RequireCompliantDevice` in Report-only mode, targeting Lab Test User with a break-glass exclusion for the admin account. The sign-in logs' **Report-only** tab (a separate tab from the live **Conditional Access** tab, specifically for policies still in Report-only state) confirmed **"Report-only: Success"** — the policy correctly evaluated the compliant Windows device and would have allowed it.

Flipping the policy to **Enforced (On)** and testing from an actually unmanaged context (my own Mac, not the VM) should have blocked the sign-in — it didn't, twice, for two separate reasons:

- **First attempt:** the sign-in (app: "OfficeHome", the M365 app launcher) succeeded despite enforcement, with the Conditional Access tab showing **"Not applied"**. Traced this to Target resources being scoped to the **"Office 365"** app bundle specifically, which doesn't necessarily include every Microsoft first-party app — "OfficeHome" wasn't covered by it. Fixed by broadening Target resources to **"All cloud apps"**.
- **Second attempt, same result.** Checked the policy's **Conditions > Device platforms** setting directly and found "Any device" was selected under Include, but under **Exclude**, Android, iOS, **macOS**, and Linux were all checked — leaving only Windows and Windows Phone actually in scope, regardless of the "Any device" label. This was a default the policy wizard applied, not something either of us intentionally configured, and it silently neutered the policy for the exact platform being tested against. Fixed by unchecking all four exclusions.

After both fixes, the Mac sign-in was genuinely blocked — Microsoft's own block screen appeared ("You can't get there from here... Your sign-in was successful, but you can't open this resource from this web browser"), and the sign-in log's live **Conditional Access** tab confirmed it: `Lab-CA-RequireCompliantDevice`, Grant control `RequireCompliantDevice`, **Result: Failure**. Checking the Device info tab on that sign-in confirmed it as the real test case: `OS: MacOS`, `Compliant: No`, `Managed: No`.
...
- A Conditional Access policy set to "Any device" under Include can still silently exclude specific platforms if the Exclude tab has boxes checked — the summary text ("Any device and 4 excluded") is easy to skim past, and the wizard can pre-check exclusions that were never intentionally chosen. Always check both Include *and* Exclude tabs on Device platforms, not just the Include summary.
- The built-in "Office 365" cloud app group in Conditional Access doesn't necessarily cover every Microsoft first-party app (it missed "OfficeHome," the app launcher) — "All cloud apps" is the safer default for a policy meant to catch everything, especially while still validating that a policy actually works.
- The sign-in log's **Report-only** tab and **Conditional Access** tab are separate — a policy in Report-only mode only shows its result under Report-only, and will show "Not applicable" under the main Conditional Access tab, which looks like a failure but isn't.
- When a Conditional Access policy doesn't behave as expected, the sign-in log's own **Device info** tab (Compliant/Managed/OS) is the fastest way to confirm exactly which device a given sign-in actually came from, rather than guessing from IP address alone — especially in a home-lab setup where a VM and its host share the same public IP.
